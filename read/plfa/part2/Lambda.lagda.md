...
open import Data.Product using (∃-syntax; Σ; _×_; _,_; proj₁; proj₂)
...
-- Your code goes here
_[_:=_]′ : Term → Id → Term → Term
subst-nat : Id → Id → Term → Term → Term
-- _[_:=_]′ is Data.Functor.Foldable.hoist subst-nat, modulo some arm-waving

(ƛ x ⇒ N                     ) [ y := V ]′ = ƛ x ⇒ subst-nat x y N V
(case L [zero⇒ M |suc x ⇒ N ]) [ y := V ]′ = case (L [ y := V ]′) [zero⇒ M [ y := V ]′ |suc x ⇒ subst-nat x y N V ]
(μ x ⇒ N                     ) [ y := V ]′ = μ x ⇒ (subst-nat x y N V)
(` x                         ) [ y := V ]′ with x ≟ y
...                                         | yes _ =  V
...                                         | no  _ =  ` x
(L · M)                        [ y := V ]′ = L [ y := V ]′ · M [ y := V ]′
...
subst-nat x y N V with x ≟ y
...                  | yes _ = N
...                  | no  _ = N [ y := V ]′

substIsCorrect : ∀ (N V : Term) (y : Id) → N [ y := V ]′ ≡ N [ y := V ]
substIsCorrect (` x) V y with x ≟ y
...                         | yes _ = refl
...                         | no  _ = refl
substIsCorrect (ƛ x ⇒ N) V y with x ≟ y
...                             | yes _ = refl
...                             | no  _ rewrite substIsCorrect N V y = refl
substIsCorrect (N · N₁) V y rewrite substIsCorrect N V y | substIsCorrect N₁ V y = refl
substIsCorrect `zero V y = refl
substIsCorrect (`suc N) V y rewrite substIsCorrect N V y = refl
substIsCorrect case N [zero⇒ N₁ |suc x ⇒ N₂ ] V y with x ≟ y
... | yes _ rewrite substIsCorrect N V y | substIsCorrect N₁ V y = refl
... | no  _ rewrite substIsCorrect N V y | substIsCorrect N₁ V y | substIsCorrect N₂ V y = refl
substIsCorrect (μ x ⇒ N) V y with x ≟ y
...                             | yes _ = refl
...                             | no  _ rewrite substIsCorrect N V y = refl
```
...
—↠≲—↠′ a b = record
  ...
  where
    to : {x y : Term} → x —↠ y → x —↠′ y
    to (_ ∎) = refl′
    to (_ —→⟨ x₁ ⟩ x₃) = trans′ (step′ x₁) (to x₃)
    trans-—↠ : {x y z : Term} → x —↠ y → y —↠ z → x —↠ z
    trans-—↠ (_ ∎) x₁ = x₁
    trans-—↠ (_ —→⟨ x ⟩ x₂) x₁ = _ —→⟨ x ⟩ trans-—↠ x₂ x₁
    from : {x y : Term} → x —↠′ y → x —↠ y
    from (step′ x₁) = _ —→⟨ x₁ ⟩ _ ∎
    from refl′ = _ ∎
    from (trans′ x₁ x₂) = trans-—↠ (from x₁) (from x₂)
    from∘to : {x y : Term} → (x—↠y : x —↠ y) → from (to x—↠y) ≡ x—↠y
    from∘to (_ ∎) = refl
    from∘to (_ —→⟨ x₁ ⟩ x—↠y) rewrite from∘to x—↠y = refl

-- Note: They are not isomorphic because —↠ is a sequence of steps, while —↠′ encodes a tree of steps.
```
...
```
-- The answer is 2. But I'd like to check it.
```
...
```
-- The answer is 6. But I'd like to check it.
```
...
```
one' : ∃[ A ] (∅ , "y" ⦂ `ℕ ⇒ `ℕ , "x" ⦂ `ℕ ⊢ ` "y" · ` "x" ⦂ A)
one' = (`ℕ , ⊢` (S′ Z) · ⊢` Z )
two' : ∀ {A} → ¬ (∅ , "y" ⦂ `ℕ ⇒ `ℕ , "x" ⦂ `ℕ ⊢ ` "x" · ` "y" ⦂ A)
two' (⊢` (S _ (S _ ())) · _)
three' : ∃[ A ] (∅ , "y" ⦂ `ℕ ⇒ `ℕ ⊢ ƛ "x" ⇒ ` "y" · ` "x" ⦂ A)
three' = (`ℕ ⇒ (proj₁ one') , ⊢ƛ (proj₂ one'))
```
...
one'' : ∀ {A B} → ¬ (∅ , "x" ⦂ A ⊢ ` "x" · ` "x" ⦂ B)
one'' (⊢` Z · ⊢` (S x ()))
two'' : Σ (Type × Type × Type) ( λ { ( A , B , C ) → (∅ , "x" ⦂ A , "y" ⦂ B ⊢ ƛ "z" ⇒ ` "x" · (` "y" · ` "z") ⦂ C) } )
two'' = (`ℕ ⇒ `ℕ , `ℕ ⇒ `ℕ , `ℕ ⇒ `ℕ) , ⊢ƛ (⊢` (S′ (S′ Z)) · (⊢` (S′ Z) · ⊢` Z))
```
