...
open import plfa.part1.Induction using (Bin; inc; to; from; law3) public; open Bin
```
...
<-trans z<s (s<s _) = z<s
<-trans (s<s cmp1) (s<s cmp2) = s<s (<-trans cmp1 cmp2)
```
...
data Trichotomous (rel : ℕ → ℕ → Set) (m : ℕ) (n : ℕ) : Set where
  mreln : rel m n → Trichotomous rel m n
  nrelm : rel n m → Trichotomous rel m n
  meqn : m ≡ n → Trichotomous rel m n

<-trichotomous : forall (m n : ℕ) → Trichotomous _<_ m n
<-trichotomous zero zero = meqn refl
<-trichotomous zero (suc n) = mreln z<s
<-trichotomous (suc n) zero = nrelm z<s
<-trichotomous (suc m) (suc n) with <-trichotomous m n
...                               | mreln m<n = mreln (s<s m<n)
...                               | nrelm n<m = nrelm (s<s n<m)
...                               | meqn  m≡n = meqn  (cong suc m≡n)
```
...
```
≤-then-< : ∀ {m n : ℕ} → suc m ≤ n → m < n
≤-then-< (s≤s z≤n) = z<s
≤-then-< (s≤s (s≤s cont)) = s<s (≤-then-< (s≤s cont))
<-then-≤ : ∀ {m n : ℕ} → m < n → suc m ≤ n
<-then-≤ z<s = s≤s z≤n
<-then-≤ (s<s cont) = s≤s (<-then-≤ cont)
...
<-trans-revisited m<n n<p = ≤-then-< (≤-trans (s≤s ≤-suc) (≤-trans (s≤s (<-then-≤ m<n)) (<-then-≤ n<p)))
```
...
o+o≡e (suc (suc em)) en = suc (suc (o+o≡e em en))
...
data One : Bin → Set where
  leadingI : One (⟨⟩ I)
  _I : {n : Bin} → One n → One (n I)
  _O : {n : Bin} → One n → One (n O)

data Can : Bin → Set where
  is0 : Can (⟨⟩ O)
  isOne : {n : Bin} → One n → Can n

inc-one : {n : Bin} → One n → One (inc n)
inc-one leadingI = leadingI O
inc-one (n O) = n I
inc-one (n I) = (inc-one n) O

inc-canonical : { n : Bin } → Can n → Can (inc n)
inc-canonical is0 = isOne leadingI
inc-canonical (isOne n) = isOne (inc-one n)

toCanonical : (n : ℕ) → Can (to n)
toCanonical zero = is0
toCanonical (suc n) = inc-canonical (toCanonical n)
...
|+|-identityˡ : {x : Bin} → Can x → x ≡ (⟨⟩ O) |+| x
|+|-identityˡ is0 = refl
|+|-identityˡ (isOne leadingI) = refl
|+|-identityˡ (isOne (_ O)) = refl
|+|-identityˡ (isOne (_ I)) = refl
...
|+|-identityʳ : {x : Bin} → Can x → x ≡ x |+| (⟨⟩ O)
|+|-identityʳ is0 = refl
|+|-identityʳ (isOne leadingI) = refl
|+|-identityʳ {n O} (isOne (_ O)) rewrite sym (lemma n) = refl
|+|-identityʳ {n I} (isOne (_ I)) rewrite sym (lemma n) = refl

|+|-one : {x y : Bin} → One x → One y → One (x |+| y)
|+|-one leadingI leadingI = leadingI O
|+|-one leadingI (n O) = n I
|+|-one leadingI (n I) = (inc-one n) O
|+|-one {n O} (m O) leadingI rewrite sym (lemma n) = m I
|+|-one (m O) (n O) = (|+|-one m n) O
|+|-one (m O) (n I) = (|+|-one m n) I
|+|-one {n I} (m I) leadingI rewrite sym (lemma n) = (inc-one m) O
|+|-one (m I) (n O) = (|+|-one m n) I
|+|-one (m I) (n I) = (inc-one (|+|-one m n)) O

|+|-canonical : {x y : Bin} → Can x → Can y → Can (x |+| y)
|+|-canonical is0 n rewrite sym (|+|-identityˡ n) = n
|+|-canonical (isOne m) (isOne n) = isOne (|+|-one m n)
|+|-canonical m is0 rewrite sym (|+|-identityʳ m) = m
...
|+|-inc (m O) (n I) = refl
|+|-inc (m I) ⟨⟩ = refl
|+|-inc (m I) (n O) rewrite |+|-inc m n = refl
|+|-inc (m I) (n I) rewrite |+|-inc m n = refl

toHomo : ∀ (m n : ℕ) → to (m + n) ≡ to m |+| to n
toHomo zero n = |+|-identityˡ (toCanonical n)
toHomo (suc m) n rewrite |+|-inc (to m) (to n) | toHomo m n = refl

doubleIsShiftL : {n : Bin} → One n → n |+| n ≡ n O
doubleIsShiftL leadingI = refl
doubleIsShiftL (n O) rewrite doubleIsShiftL n = refl
