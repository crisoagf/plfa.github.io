...
```
∸-absorption : ∀ (n : ℕ) → zero ∸ n ≡ zero
∸-absorption zero = refl
∸-absorption (suc n) = refl
```
...
∸+assoc : ∀ (m n p : ℕ) → m ∸ n ∸ p ≡ m ∸ (n + p)
∸+assoc _ zero _ = refl
∸+assoc zero (suc _) zero = refl 
∸+assoc zero (suc _) (suc _) = refl 
...
^-distribˡ-|-* m 0 p = sym (+-identity′ (m ^ p))
^-distribˡ-|-* m (suc n) p rewrite ^-distribˡ-|-* m n p = sym (*-assoc m (m ^ n) (m ^ p))
^-distribʳ-|-* : ∀ (m n p : ℕ) → (m * n) ^ p ≡ (m ^ p) * (n ^ p)
^-distribʳ-|-* m n 0 = refl
^-distribʳ-|-* m n (suc p) rewrite ^-distribʳ-|-* m n p = begin
  m * n * ((m ^ p) * (n ^ p))
...
^-*-assoc : ∀ (m n p : ℕ) → (m ^ n) ^ p ≡ m ^ (n * p)
^-*-assoc _ 0 0 = refl
^-*-assoc m 0 (suc p) rewrite ^-*-assoc m 0 p = refl
^-*-assoc m (suc n) p = begin
  (m * (m ^ n)) ^ p
  ≡⟨ ^-distribʳ-|-* m (m ^ n) p ⟩
  m ^ p * (m ^ n) ^ p
...
law1 ⟨⟩ = refl
law1 (n O) = refl
law1 (n I) rewrite law1 n
                 | +-assoc (from n) (suc (from n)) 0
                 | +-comm (from n) (suc (from n + 0))
                 | +-comm (from n) 0 = refl
law2ce : to (from ⟨⟩) ≡ ⟨⟩ O
law2ce = refl
law3 : forall (n : ℕ) → from (to n) ≡ n
law3 zero = refl
